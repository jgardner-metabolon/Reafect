...
class Reafect:
    def __init__(
        ...
        self.kegg_enzyme_with_reactions = pd.read_csv(
            path_to_databases + "kegg_enzyme_with_reaction.csv"
    ...
    def update_kegg_pathways(self, extra_reactions, custom_ID_to_KEGG):
        ...
        for module_or_pathway_ID in all_Ms:
            ...
            if os.path.exists(
                self.path_to_store_maps + "{}ext.pickle".format(module_or_pathway_ID)
            ):
                print(
                    " Map {m} has already been updated with additional reactions, delete {m}ext.pickle if you want to redo update.".format(
                        m=module_or_pathway_ID
                    )
                )
                continue
            ...
            # Load edges
            with open(
                self.path_to_store_maps + "{}.pickle".format(module_or_pathway_ID), "rb"
            ) as f:
            ...
            if (N_before - N_after) != 0:
                print(
                    "Module/pathway {} has been updated. #edges before {}, #edges after {}.".format(
                        module_or_pathway_ID, N_before, N_after
                    )
                )

                with open(
                    self.path_to_store_maps
                    + "{}ext.pickle".format(module_or_pathway_ID),
                    "wb",
                ) as handle:
    ...
    def prepare_pathways(self):
        ...
        if os.path.exists(file):
            print(
                "Pathways already prepared. Delete {} if you want to redo pathway preparation. You should do this when you manually added more reactions.".format(
                    file
                )
            )
            ...
            for module_or_pathway_ID in all_Ms:
                ...
                with open(
                    "{}/{}.pickle".format(
                        self.path_to_store_maps, module_or_pathway_ID.strip(".pickle")
                    ),
                    "rb",
                ) as f:
                    M_edges = pickle.load(f)
                ...
                G_edges = [
                    (clean_element(el[0]), clean_element(el[1])) for el in M_edges
                ]
                ...
                ms = [
                    el
                    for el in G
                    if not ((("R" in el) and ("RM_" not in el)) or ("RR_" in el))
                ]
                # Get all reaction nodes in the graph
                Rs = [
                    el
                    for el in G
                    if ((("R" in el) and ("RM_" not in el)) or ("RR_" in el))
                ]
                    ...
                    for pair in all_pairs:
                        ...
                        for cuttoff in [3, 5, 7, 9, 11, 13, 15]:
                            ...
                            for ii, path in enumerate(
                                nx.all_simple_paths(
                                    ego, source=s, target=t, cutoff=cuttoff
                                )
                            ):
                                ...
                                for edge in edges:
                                    ...
                                    if (edge in G_edges) and (
                                        (edge[1], edge[0]) in G_edges
                                    ):
                                ...
                                prod_pos = np.prod(decay_pos)
                                prod_neg = np.prod(decay_neg)
                                if isinstance(sym_vars[s], tuple):
                                    prod_pos = np.prod([prod_pos, *sym_vars[s]])
                                    prod_neg = np.prod([prod_neg, *sym_vars[s]])
                                else:
                                    prod_pos *= sym_vars[s]
                                    prod_neg *= sym_vars[s]
                                decay_paths.append(
                                    {
                                        "decay_pos": prod_pos,
                                        "decay_neg": prod_neg,
                                        "last_step_direct": sign_stream[
                                            -1
                                        ],  # This indicates if the connection is on the up- or downstream side of the reaction
    ...
    def determine_pathways_for_abc_params(
        self, a_params=[0.85], b_params=[0.35], c_params=[0.85], n_cores=4
    ):
        ...
        if cnt > 0:
            raise ValueError(
                "Wrong input for a_params, b_params, c_params. Numbers need to be between 0 and 1."
            )
        ...
        def subs_abc(a_param, b_param, c_param):
            ...
            df_decay_paths_abc["decay_pos"] = df_decay_paths_abc["decay_pos"].apply(
                subs_ab
            )
            df_decay_paths_abc["decay_neg"] = df_decay_paths_abc["decay_neg"].apply(
                subs_ab
            )

            with open(
                self.path_to_abc_E
                + "all_m_R_E_{a}_{b}_{c}.pickle".format(
                    a=a_param, b=b_param, c=c_param
                ),
                "wb",
            ) as handle:
                pickle.dump(
                    df_decay_paths_abc, handle, protocol=pickle.HIGHEST_PROTOCOL
                )
        ...
        for a_param in a_params:
            for b_param in b_params:
                for c_param in c_params:
                    file = self.path_to_abc_E + "all_m_R_E_{a}_{b}_{c}.pickle".format(
                        a=a_param, b=b_param, c=c_param
                    )
                    ...
                    if not os.path.exists(file):
                        outputs.append(
                            dask.delayed(subs_abc)(a_param, b_param, c_param)
                        )
    ...
    def process_patient(
        ...
        patient_ID,
        Z_score_data,
        type_of_input_Z_scores="Z_scores",
        disease_enzyme=None,
        a_param=0.85,
        b_param=0.35,
        c_param=0.75,
        custom_reactions_with_enzyme=pd.DataFrame([], columns=["enzyme", "R"]),
    ):

        # Check input of custom_reactions_with_enzyme
        assert isinstance(custom_reactions_with_enzyme, pd.DataFrame)
        assert (
            len(set(custom_reactions_with_enzyme.columns).intersection(["enzyme", "R"]))
            == 2
        )
        ...
        file = (
            path_to_sample
            + "SR_scores_{a}_{b}_{c}_{type_of_input_Z_scores}.csv".format(
                a=a_param,
                b=b_param,
                c=c_param,
                type_of_input_Z_scores=type_of_input_Z_scores,
            )
        )
        if not os.path.exists(file):
            ...
            df_pat_abc = self.df_decay_paths_abc.merge(
                Z_score_data.reset_index().rename(columns={"network_ID": "m"}), on="m"
            )

            SR_scores = []
            gb = df_pat_abc.groupby(by=["maps", "R"])
            for i, (((module_pathway, R), data_R), progress) in enumerate(
                zip(gb, progressbar(range(len(gb)), "Computing: ", 60))
            ):
                ...
                if not data_R_pos.empty:
                    ...
                    weighted_E_pos = data_R_pos.groupby(by=["m"]).apply(
                        lambda x: x[["decay_pos"]] ** 2 / x[["decay_pos"]].abs().sum()
                    )
                    weighted_E_pos = weighted_E_pos.rename(
                        columns={"decay_pos": "decay_pos_weighted"}
                    )
                    data_R = data_R.assign(
                        decay_pos_weighted=weighted_E_pos["decay_pos_weighted"]
                    )
                else:
                ...
                if not data_R_neg.empty:
                    # Group by metabolite and calculate normalized Z-score for every path
                    weighted_E_neg = data_R_neg.groupby(by=["m"]).apply(
                        lambda x: x[["decay_neg"]] ** 2 / x[["decay_neg"]].abs().sum()
                    )
                    weighted_E_neg = weighted_E_neg.rename(
                        columns={"decay_neg": "decay_neg_weighted"}
                    )
                    data_R = data_R.assign(
                        decay_neg_weighted=weighted_E_neg["decay_neg_weighted"]
                    )
                ...
                for up_down, data_up_down in data_R.groupby("last_step_direct"):
                    ...
                    if up_down == 1:
                        SR_up = sum(data_up_down["decay_pos_weighted"].dropna()) - sum(
                            data_up_down["decay_neg_weighted"].dropna()
                        )
                        SR += SR_up
                    ...
                    if up_down == -1:
                        SR_down = sum(
                            data_up_down["decay_neg_weighted"].dropna()
                        ) - sum(data_up_down["decay_pos_weighted"].dropna())
                        SR += SR_down
                ...
                if len(uni_rev_irrev) > 1:
                    ...
                    SR_scores.append(
                        [R, SR, list(data_R["m"].unique()), module_pathway, "irrev"]
                    )

                # reversible reaction than absolute value of SR
                elif uni_rev_irrev[0] == "rev":
                    SR_scores.append(
                        [R, abs(SR), list(data_R["m"].unique()), module_pathway, "rev"]
                    )

                # Check what went wrong!
                else:
                    SR_scores.append(
                        [R, SR, list(data_R["m"].unique()), module_pathway, "error"]
                    )

            SR_scores = pd.DataFrame(
                SR_scores, columns=["R", "SR", "ms", "maps", "irrev_rev"]
            )
            SR_scores["SR"] = SR_scores["SR"].apply(np.float64)
            SR_scores = SR_scores.sort_values(by="SR", ascending=False).reset_index(
                drop=True
            )
            ...
            kegg_enzyme_with_reactions = pd.concat(
                [kegg_enzyme_with_reactions, custom_reactions_with_enzyme]
            )
            kegg_enzyme_with_reactions = kegg_enzyme_with_reactions.loc[
                ~kegg_enzyme_with_reactions.duplicated(keep="first")
            ]
            kegg_enzyme_with_reactions = (
                kegg_enzyme_with_reactions.groupby("enzyme")
                .apply(lambda x: x["R"].tolist())
                .to_dict()
            )
            ...
            for enz, Rs in kegg_enzyme_with_reactions.items():
                ...
                if len(s) > 4:
                    # Search matches
                    R_matches = unique_R.loc[
                        unique_R["R"].str.contains(s), "R"
                    ].tolist()
                    for R in R_matches:
            ...
            SR_scores_enz = SR_scores_enz.sort_values(
                by=["SR", "enzyme"], ascending=False
            ...
            print(
                "Results are already determined and stored. Delete file if you want to redo processing."
            )
            return SR_scores_enz
